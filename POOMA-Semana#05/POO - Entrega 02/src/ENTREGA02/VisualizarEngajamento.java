package ENTREGA02;

public class VisualizarEngajamento {

    private int idEngajamento;
    private int engajamentoTotal;

    public int getIdEngajamento() {
        return idEngajamento;
    }

    public void setIdEngajamento(int idEngajamento) {
        this.idEngajamento = idEngajamento;
    }

    public int getEngajamentoTotal() {
        return engajamentoTotal;
    }

    public void setEngajamentoTotal(int engajamentoTotal) {
        this.engajamentoTotal = engajamentoTotal;
    }

    public VisualizarEngajamento () {}

    public static void Engajmaento (int idEngajamento, int engajamentoTotal) {
        
    }

    public static void GerarRelatorioEngajamento (String cliente) {

    }

    public static void RelatorioEngajamento (String cliente) {

    }
}
